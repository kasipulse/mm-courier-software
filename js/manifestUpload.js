...
import { collection, doc, setDoc, writeBatch } from "https://www.gstatic.com/firebasejs/10.14.0/firebase-firestore.js";
...
document.getElementById("uploadBtn").addEventListener("click", async () => {
  ...
  for (let file of files) {
    ...
    data[type] = parsed;
    if (!manifestId) manifestId = file.name.split(/[\-_]/)[0]; // "JNB-873535"
  }
  ...
  for (let wb of data.waybill) {
    ...
    batch.set(docRef, { ...wb, manifestId, createdAt: new Date().toISOString() });
  }
  await batch.commit();
